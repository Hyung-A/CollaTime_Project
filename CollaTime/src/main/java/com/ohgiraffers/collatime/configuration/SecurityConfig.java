...
//            auth.requestMatchers("/auth/login", "user/signup", "/", "main").permitAll();
...
//        return http.build();
//    }
    //    관리자와 유저의 디폴트 페이지를 나누려면 2개의 필터체인을 만들면 해결할 수 있다.
//    @Order(1)
//    @Bean
//    public SecurityFilterChain userConfigure(HttpSecurity http) throws Exception {
////        요청 권한 체크
//        http.authorizeHttpRequests( auth -> {
////            권한이 없어도 들어갈 수 있는 페이지
//            auth.requestMatchers("/auth/login", "user/signup", "/", "main").permitAll();
...
//            login.passwordParameter("pass");
////            로그인에 성공했을 때 들어갈 기본 페이지
//            login.defaultSuccessUrl("/project/ProjectMain", true);
//            login.failureHandler(authFailHandler);
//        }).logout( logout -> {
////            로그아웃 요청에 대한 매핑
//            logout.logoutRequestMatcher(new AntPathRequestMatcher("/auth/logout"));
